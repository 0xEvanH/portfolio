...
const ProjectCard: FC<{ project: Project; cardIndex: number; sectionVisible: boolean }> = ({
  project,
  cardIndex,
  sectionVisible,
}) => {
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background: `radial-gradient(circle at ${tilt.lightX}% ${tilt.lightY}%, rgba(255,255,255,0.07) 0%, transparent 65%)`,
      ...
      <div className="absolute inset-0 overflow-hidden">
        <img
          ...
          style={{
            opacity: imageLoaded ? (tilt.isHovered ? 0.42 : 0.18) : 0,
        ...
        <div
          ...
          style={{ background: "linear-gradient(160deg, transparent 15%, rgba(0,0,0,0.9) 100%)" }}
        />
      </div>
      ...
      <div className="relative z-2 h-full flex flex-col justify-between p-6">
        <div className="flex justify-between items-start">
          <span
            className="uppercase"
            style={{ fontWeight: 700, fontSize: "0.58rem", letterSpacing: "0.12em", color: project.accent, opacity: 0.85 }}
          >
            {project.year}
          </span>
          <a
            ...
            className="w-8 h-8 rounded-lg flex items-center justify-center no-underline text-white backdrop-blur-2"
            style={{
        ...
        <div>
          <div
            className="mb-[0.4rem]"
            style={{ fontWeight: 400, fontSize: "0.6rem", letterSpacing: "0.1em", color: "rgba(255,255,255,0.2)" }}
          >
            {project.id}
          </div>
          <h3
            ...
            style={{
              ...
              fontSize: isWide ? "clamp(1.4rem, 2.5vw, 2rem)" : "clamp(1.1rem, 2vw, 1.5rem)",
          ...
          <p
            className="m-0 mb-[1.1rem] max-w-[42ch]"
            style={{
              fontWeight: 400,
              fontSize: "0.78rem",
              lineHeight: 1.75,
              color: "rgba(255,255,255,0.5)",
              opacity: (tilt.isHovered || isTall) ? 1 : 0,
              transform: (tilt.isHovered || isTall) ? "translateY(0)" : "translateY(8px)",
          ...
          <div className="flex gap-[0.4rem] flex-wrap">
            {project.stack.map((tag, tagIndex) => (
              <span
                key={tag}
                className="rounded-full py-1 px-[0.65rem]"
                style={{
                  fontWeight: 600,
                  fontSize: "0.58rem",
                  letterSpacing: "0.06em",
                  background: tilt.isHovered ? `${project.accent}1a` : "rgba(255,255,255,0.06)",
                  border: tilt.isHovered ? `1px solid ${project.accent}40` : "1px solid rgba(255,255,255,0.09)",
                  color: tilt.isHovered ? project.accent : "rgba(255,255,255,0.5)",
                  transition: `all 0.3s ease ${tagIndex * 45}ms`,
                  transform: tilt.isHovered ? "translateY(0)" : "translateY(2px)",
                }}
              >
                {tag}
              </span>
...
const Work: FC = () => {
  ...
  return (
    <section id="work" ref={ref} className="relative z-1 min-h-screen px-20 py-32">
      <ScrollRevealBlock delay={0}>
        <div className="flex justify-between items-end mb-12">
          ...
          <span
            className="uppercase"
            style={{ fontWeight: 400, fontSize: "0.7rem", letterSpacing: "0.1em", color: "rgba(255,255,255,0.38)" }}
          >
            {projects.length} projects
          </span>
        </div>
      </ScrollRevealBlock>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gridAutoRows: "260px", gap: "1rem" }}>
        {projects.map((project, i) => (
          <ProjectCard key={project.id} project={project} cardIndex={i} sectionVisible={sectionVisible} />
        ))}
      </div>
    </section>
  );
