...
const StatBlock: FC<{ value: number; label: string }> = ({ value, label }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className="uppercase mt-[0.3rem]"
        style={{
          fontWeight: 400,
          fontSize: "0.68rem",
          letterSpacing: isHovered ? "0.18em" : "0.1em",
          color: isHovered ? "#ffffff" : WHITE_DIM,
...
const Hero: FC<{ ready: boolean }> = ({ ready }) => {
  ...
  return (
    <section
      ref={sectionRef}
      id="home"
      className="relative z-1 min-h-screen flex flex-col justify-end px-20 pb-20"
    >
      <div
        className="uppercase mb-8"
        style={{ fontWeight: 400, fontSize: "0.7rem", letterSpacing: "0.14em", color: WHITE_DIM, ...entranceStyle(0.05) }}
      >
      ...
      <div style={{
        ...entranceStyle(0.1),
        transform: `${ready ? "translateY(0)" : "translateY(28px)"} translate(${parallaxX * 0.4}px, ${parallaxY * 0.4}px)`,
        transition: "opacity 0.85s ease 0.1s, transform 0.85s cubic-bezier(0.16,1,0.3,1) 0.1s",
      }}>
        <FlickerHeading
          text={nameText}
          tag="h1"
          style={{ fontWeight: 900, fontSize: "clamp(4.5rem, 14vw, 12rem)", lineHeight: 0.88, letterSpacing: "-0.04em", color: "#ffffff", margin: "0 0 0.06em" }}
        />
      </div>

      <div style={{
        ...entranceStyle(0.18),
        transform: `${ready ? "translateY(0)" : "translateY(28px)"} translate(${parallaxX * 0.65}px, ${parallaxY * 0.65}px)`,
        transition: "opacity 0.85s ease 0.18s, transform 0.85s cubic-bezier(0.16,1,0.3,1) 0.18s",
      }}>
        <FlickerHeading
          text={fieldText}
          tag="h2"
          style={{ fontWeight: 900, fontSize: "clamp(4.5rem, 14vw, 12rem)", lineHeight: 0.88, letterSpacing: "-0.04em", color: "rgba(255,255,255,0.18)", margin: "0 0 3.5rem" }}
        />
      </div>
